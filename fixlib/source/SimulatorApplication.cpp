...
static void sendMarketDataRequestReject(const std::string& req_id, const std::string& error_msg, const FIX::SessionID& session_id);
static void sendMarketDataSnapshot(const MarketData&, const FIX::SessionID& session_id);


SimulatorApplication::SimulatorApplication(const FIX::SessionSettings& s) 
	: settings(s), subscription_manager( new SubscriptionManager )
{
	initMarketData();
}
...
void SimulatorApplication::onMessage( const FIX44::MarketDataRequest& m, const FIX::SessionID& session)
{
	...
	try{
		...
		BOOST_FOREACH(const std::string& symbol, symbols)
		{
			if (FIX::SubscriptionRequestType_SNAPSHOT_PLUS_UPDATES == subscription_type)
			{
				MarketData md = getMarketDataForSymbol(symbol);
			...
			else if(FIX::SubscriptionRequestType_SNAPSHOT_PLUS_UPDATES == subscription_type)
			{
...
static void sendMarketDataRequestReject(const std::string& req_id, const std::string& error_msg, const FIX::SessionID& session) {
...
static void sendMarketDataSnapshot(const MarketData& md, const FIX::SessionID& session_id)
{
...
void SimulatorApplication::initMarketData(){
	...
	FIX::Locker l(market_mutex);
	BOOST_FOREACH(const std::string& instrument, instruments)
	{
		MarketData_ptr p(new MarketData(instrument));
		// generate initial bid and offer
		double bid = double(rand()%10 + 1) *1000.0;
		p->set_bid_quantity(1000);
		p->set_offer_quantity(1000);
		p->set_bid_price(bid);
		p->set_offer_price(bid*1.1);
		markets.push_back(p);
...
{
	size_t index = rand()%markets.size();
	MarketData_ptr p = markets[index];
	simulateMarketEvent(p);
}

void SimulatorApplication::simulateMarketEvent(MarketData_ptr p){
	// modify bid and offer
	MarketData md;
	{
		FIX::Locker l(market_mutex);
 		double r = double(rand())/double(RAND_MAX);	// 0.0 - 1.0
 		double bid =  p->get_bid_price() * (0.9+r*0.2);
 		p->set_bid_price(bid);
 		p->set_offer_price(bid*1.1);
		md = *p;
	}
...
// returns a *copy* of the market data for a given symbol
MarketData SimulatorApplication::getMarketDataForSymbol(const std::string& symbol)
{
	MarketData md;
...
bool SimulatorApplication::symbolExists(const std::string& symbol) const
{
